...
const DIVISIONS = [
  ...
  { divisionId: 1000, divisionName: "슈퍼챌린지" },
  ...
  { divisionId: 2600, divisionName: "세미프로1" },
  { divisionId: 2700, divisionName: "세미프로2" },
  { divisionId: 2800, divisionName: "세미프로3" },
  { divisionId: 2900, divisionName: "유망주1" },
  { divisionId: 3000, divisionName: "유망주2" },
  { divisionId: 3100, divisionName: "유망주3" },
];

export function getDivisionName(divisionId: number): string {
  const found = DIVISIONS.find((d) => d.divisionId === divisionId);
  return found ? found.divisionName : `Unknown(${divisionId})`;
}
